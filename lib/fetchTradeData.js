...
async function fetchData() {
    ...
    try {
        ...
        if(APIcalls <= maxAPICalls) {
            ...
            log.info(err);
            log.error("Error: couldn't get a response from Kraken API despite " + (APIcalls-1) + " calls.");
            process.exit(0);
        }
        return;
    }
    ...
    historicalData.map(el => {
        ...
        tradeDataModel.findOne( { pair: pair, interval: interval, time: time }, (err, doc) => {
            if(doc) {
...
setInterval(() => { 
    if(recordsProcessed === recordsToProcess) {
        log.info(`Successfully fetched data for ${pair}_${interval}. ${newRecords} new records.`)
        process.exit(0);
    }
}, 500);